...
<body>
    ...
    if (isset($_SESSION["gerichtid"])){
        ...
        $sql = "SELECT b.email, b.admin, bbg.Sternbewertung, bbg.Bemerkung, bbg.Hervorgehoben, bbg.Bewertungszeitpunkt, bbg.id as Bewertungsid
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                if ($row['Hervorgehoben'] == 1){
                    echo '<tr><td><b>' . $row['email'] . '</b></td><td><b>' . $row['Sternbewertung'] . ' Sterne</b></td><td><b>' .
                        $row['Bemerkung'] . '</b><td><b><a href="/bewertung?nothighlight=true&bewertungid='. $row['Bewertungsid']. '">Hervorhebung abwählen</a></b></td></tr>';
                }
                else{
                    echo '<tr><td>' . $row['email'] . '</td><td>' . $row['Sternbewertung'] . ' Sterne</td><td>' .
                        $row['Bemerkung'] . '<td><a href="/bewertung?highlight=true&bewertungid='. $row['Bewertungsid']. '">Hervorheben</a></td></tr>';
                }
