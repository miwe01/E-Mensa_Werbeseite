    ...
    <div id="speise-wrapper">
        <div id="speisekarte">
          ...
          GROUP BY gericht.name LIMIT 5;";
            ...
            <table class="allergen-table">
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    if (in_array($row['code'],$allergenList)) {
                        echo '<tr><td class="allergen-td"><sub><b>'.$row['code'].'</b></sub></td><td class="allergen-td">'.$row['allergenname']. '</td></tr>';
                    }
                }
            ...
            <!-- karte ende -->
            <!-- Liste der neusten 5 Wunschgerichte -->
            ...
            ORDER BY erstellungsdatum DESC LIMIT 5";
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr><td class="wunschspeisen-td">'.$row['wunschgericht'].'</td><td class="wunschspeisen-td">'.$row['beschreibung']. '</td><td class="wunschspeisen-td">'.$row['ersteller'].'</td></tr>';
                }
...
@section('meinungen')
    <!-- start Meinungen der Gäste -->
    <div id="meinungen-wrapper">
        <div id="speisekarte">
        <h2 id="h2-meinungen">Meinungen unserer Gäste</h2>
        <table class="speisen-table">
            <tr>
                <th class="speisen-td-th speisen-th">Speise</th>
                <th class="speisen-td-th speisen-th">Bemerkung</th>
                <th class="speisen-td-th speisen-th">Bewertung</th>
            </tr>
        <?php
        //Hervorgehobene Bewertungen abrufen
        $link = mysqli_connect("localhost", // Host der Datenbank
            "root",                 // Benutzername zur Anmeldung
            "test..123",    // Passwort
            "emensawerbeseite",     // Auswahl der Datenbanken (bzw. des Schemas)
        ...
        $sql = "SELECT g.name as gerichtname, Sternbewertung, Bemerkung
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr><td class="wunschspeisen-td">'.$row['gerichtname'].'</td>
                        <td class="wunschspeisen-td">'.$row['Bemerkung']. '</td>
                        <td class="wunschspeisen-td">' . $row['Sternbewertung'] . ' Sterne</td></tr>';
        }
